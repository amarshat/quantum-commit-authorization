...
Residual exposure: a reveal that expires unincluded (fee spike, censorship) leaves its secret public with its commitment dead (the LEAKED-LIVE state). The leaf is now a bearer authorization for anyone who saw the calldata. The wallet must treat it as burned and never recommit it, and should call `burn` to nullify the leaf on-chain. But `burn` is age-gated (see burn-griefing below), so it is not a free win: the holder commits-to-burn and ages exactly as an attacker commits-to-steal and ages, a symmetric race. GAME.md Theorem 2' proves this is unavoidable, because in the leaked state nothing on-chain tells the holder apart from an attacker. So the real defense is to not reach LEAKED-LIVE at all: do not broadcast a reveal until its commit is final. Recording which leaves were exposed is off-chain state that chain-scanning cannot reconstruct (see state loss), so it must be persisted write-ahead, before the reveal is broadcast.
...
### Burn-griefing (and why burn is age-gated)

A reveal in the public mempool exposes `(secret, proof)`, which is exactly the input `burn` needs. If `burn` had no age requirement, any observer could copy those out of a pending reveal, submit an immediate `burn`, order it ahead of the reveal, and nullify the leaf, denying the action for the cost of one transaction with no censorship. It moves no funds, but it is a repeatable denial-of-service on every reveal, and since rotation is a revealed action it would also let an attacker block the break-glass response to seed compromise indefinitely. This was a real hole in the first design. `burn` therefore requires its own aged commitment, domain-separated by `TAG_BURN`, exactly like reveal: an attacker who only learns the secret at reveal time has no aged burn commitment, so the victim's already-aged reveal wins, and the griefing burn is demoted to the same aged-commitment race as theft. The unavoidable cost is that recovering a genuinely leaked leaf is now itself a race (see reveal-phase theft residual exposure, and GAME.md Section 6-7). This closes the DoS at the price of race-free recovery, which GAME.md Theorem 2' shows was impossible to keep anyway.
...
- Commit reorged out, reveal already broadcast: this is a theft window, not just a lost leaf (the LEAKED-LIVE state). The reveal reverts because the commitment is gone (the existence check precedes the nullifier write), so the leaf is never nullified on-chain, yet the secret is now public. Any observer can post a fresh commitment for a theft action on that live leaf and, after `minCommitAge`, take it; the victim no longer has an aged-commitment advantage. Defense: do not broadcast a reveal until its commit is final (see the `minCommitAge` two-budget note in the spec). If a secret does leak this way, the holder can commit-to-burn and nullify the leaf, but that is a symmetric race against any thief, not a free save (burn is age-gated; see burn-griefing above and GAME.md Theorem 2). Avoiding the leak by waiting for commit finality is the only race-free option.
